...
const ProjectView: React.FC = () => { // Removed props
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (formData.name && formData.projectLeader) {
      try {
        if (editingProject) {
          await dataService.updateProject(editingProject.id, formData);
          ...
          showNotification('แก้ไขข้อมูลโครงการสำเร็จ', 'success');
        } else {
                        ...
                        {(userRole === 'admin' || userRole === 'officer') && (
                          <>
                            {proj.status === 'Completed' && (
                              <button
                                onClick={(e) => { e.stopPropagation(); handleOpenReport(proj); }}
                            ...
                            <button
                              onClick={(e) => { e.stopPropagation(); confirmDeleteProject(proj); }}
