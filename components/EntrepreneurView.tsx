    ...
    {data.map(ent => (
      <div key={ent.id} className="bg-white/80 backdrop-blur-sm border border-slate-200 rounded-xl shadow-lg hover:shadow-blue-500/10 hover:-translate-y-1 transition-all duration-300 flex flex-col group">
        <div className="p-6">
          ...
          <p className="text-base text-slate-600 mb-4">ผู้ติดต่อ: <span className="font-semibold text-slate-800">{ent.name} {ent.nickname && <span className="text-slate-500 font-normal text-sm">({ent.nickname})</span>}</span></p>
    ...
    {data.map((ent, index) => (
      <div key={ent.id} className={`p-4 flex flex-col md:flex-row md:items-center gap-4 ${index < data.length - 1 ? 'border-b border-slate-200' : ''}`}>
        <div className="flex-1">
          ...
          <p className="text-base text-slate-500">ผู้ติดต่อ: {ent.name} {ent.nickname && <span className="text-slate-400 text-sm">({ent.nickname})</span>}</p>
        </div>
      ...
      <Modal isOpen={isFormModalOpen} onClose={handleCloseModals} title={editingEntrepreneur ? 'แก้ไขข้อมูลผู้ประกอบการ' : 'เพิ่มผู้ประกอบการใหม่'}>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <input type="text" placeholder="ชื่อเล่น (ไม่บังคับ)" value={formData.nickname || ''} onChange={e => setFormData({ ...formData, nickname: e.target.value })} className="w-full px-4 py-3 bg-slate-50 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white transition-colors" />
            ...
            <div className="space-y-3 text-base text-slate-700">
              ...
              <div className="flex items-start gap-4 p-2 rounded-lg bg-slate-50">
                ...
                <div>
                  ...
                  <span>{viewingEntrepreneur.name} {viewingEntrepreneur.nickname && <span className="text-slate-500 text-sm">({viewingEntrepreneur.nickname})</span>}</span>
                </div>
              </div>
              <div className="flex items-start gap-4 p-2 rounded-lg">
