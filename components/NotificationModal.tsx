...
const NotificationModal: React.FC<NotificationModalProps> = ({ isOpen, onClose, message, type }) => {
  ...
  return (
    <div
      className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[200] flex justify-center items-center p-4 animate-fade-in"
      style={{ animationDuration: '0.3s' }}
    >
      <div
        className="bg-white border border-slate-100 rounded-2xl shadow-2xl w-full max-w-sm animate-scale-in text-center p-8"
      >
        <div className="flex justify-center mb-4">
          {icons[type]}
        </div>
        <h2 className="text-2xl font-medium font-title text-slate-900 mb-2">{titleText[type]}</h2>
        ...
        {(type === 'success' || type === 'delete') ? (
          ...
          <button
            onClick={onClose}
            className={`w-full px-6 py-3 rounded-lg text-white font-semibold transition-colors ${buttonColors.error} focus:outline-none focus:ring-2 focus:ring-offset-2 font-body`}
          >
            ตกลง
          </button>
        )}
